...
<form method="post" onsubmit="return confirmInsert();">
    ...
    <input type="number" step="0.01" id="WorkshopID" name="WorkshopID" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO enrollment (EnrollmentID, WorkshopID, AttendeeID, EnrollmentDate) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("iiis", $EnrollmentID, $WorkshopID, $AttendeeID, $EnrollmentDate);
...
<?php
// No need to include db_connection.php again
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $EnrollmentID = $row['EnrollmentID']; // Corrected variable name
        echo "<tr>
            <td>" . $row['EnrollmentID'] . "</td>
            <td>" . $row['WorkshopID'] . "</td>
            <td>" . $row['AttendeeID'] . "</td>
            <td>" . $row['EnrollmentDate'] . "</td>
            <td><a style='padding:4px' href='delete_enrollment.php?EnrollmentID=$EnrollmentID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_enrollment.php?EnrollmentID=$EnrollmentID'>Update</a></td> 
        </tr>";
    }
