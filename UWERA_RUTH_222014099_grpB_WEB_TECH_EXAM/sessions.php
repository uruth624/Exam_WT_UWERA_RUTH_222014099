...
<form method="post" onsubmit="return confirmInsert();">
    ...
    <label for="SessionTitle">SessionTitle:</label>
    <input type="text" id="SessionTitleorder_date" name="SessionTitle" required><br><br>

    <label for="SessionDescription ">SessionDescription :</label>
    <input type="number" id="SessionDescription " name="SessionDescription " required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO sessions ( SessionID, WorkshopID , SessionTitle,  SessionDescription , StartTime ,EndTime) VALUES (?, ? ,? , ?, ?, ?)");
    $stmt->bind_param("iiss", $SessionID, $WorkshopID,$SessionTitle, $SessionDescription,$StartTime, $EndTime);
    // Set parameters and execute
    $SessionID= $_POST['SessionID'];
    $orkshopID  = $_POST[ 'WorkshopID '];
...
<body>
    <center><h2>Table of sessions</h2></center>
    <table border="3">
        <tr>
            <th>SessionID</th>
            <th>WorkshopID</th>
            <th>SessionTitle</th>
            <th>SessionDescription</th>
            <th>StartTime</th> <!-- Corrected column header -->
            <th>EndTime</th>
            <th>Delete</th>
            <th>Update</th>
        </tr>
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $SessionID = $row['SessionID']; // Fetch SessionID
        echo "<tr>
            <td>" . $row['SessionID'] . "</td>
            <td>" . $row['WorkshopID'] . "</td>
            <td>" . $row['SessionTitle'] . "</td>
            <td>" . $row['SessionDescription'] . "</td> <!-- Removed extra space -->
            <td>" . $row['StartTime'] . "</td> <!-- Corrected column name -->
            <td>" . $row['EndTime'] . "</td>
            <td><a style='padding:4px' href='delete_sessions.php?SessionID=$SessionID'>Delete</a></td> <!-- Corrected link -->
            <td><a style='padding:4px' href='update_sessions.php?SessionID=$SessionID'>Update</a></td> <!-- Corrected link -->
            </tr>";
    }
